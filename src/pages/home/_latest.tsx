...
const DocumentIcon = styled(FontAwesomeIcon)`
  margin-left: 1rem;
`
const SoundIcon = styled(FontAwesomeIcon)`
  margin-left: 1rem;
`
...
const LatestOpinion = (props: OpinionData) => {
  ...
  return (
    <>
      ...
      <Text size="small">
        {dateString} {props.status && statusString} {docketString}{' '}
        {props.natureSuit && natureString}
      </Text>
    </>
  )
}
...
export const LatestOpinionList = () => {
  const { data, isLoading }: QueryResult<OpinionApiResponse, {}> = useQuery(
    'latestOpinions',
    fetchLatestOpinion
  )
