...
export default class HtmlTemplate extends React.Component<HtmlProps> {
  public static async getInitialProps({ assets, data, renderPage }: CtxProps) {
    ...
    const page = await renderPage((App: React.FC) => (props: any) =>
    ...
    return (
      <html {...htmlAttrs}>
        <head>
          ...
          <link
            rel="stylesheet"
            href="http://fonts.googleapis.com/css?family=Roboto:100,100italic,300,300italic,400,400italic,500,500italic,700&display=swap"
          />
          <link
            rel="stylesheet"
            href="http://fonts.googleapis.com/css?family=Roboto+Slab&display=swap"
          />
          {styleTags}
        </head>
        <body {...bodyAttrs}>
