...
export default () => {
  ...
  return (
    <Header direction="column" pad="small" align="end" background={darkMode ? 'dark-2' : 'light-4'}>
      <Box direction="row" pad="small" fill="horizontal">
        <Box align="center" alignSelf="start" margin={{ right: 'auto' }}>
          <Box direction="row">
            <Banner level={1} margin="none" color="neutral-2">
              Court
            </Banner>
            <Banner level={1} margin="none" color="accent-2">
              Listener
            </Banner>
          </Box>
        ...
        <Button hoverIndicator="background" onClick={() => toggleMode()} alignSelf="start">
          <Toggle toggle reverse color="dark-5" onChange={() => toggleMode()} />
        </Button>
      </Box>
      <OptionsBox />
    </Header>
